...
def partition_classes(X, y, split_attribute, split_val):
    ...
    if isinstance(split_val, int):
        for rowx, rowy in zip(X,y):
            if rowx[split_attribute]<=split_val:
                X_left.append(rowx)
                y_left.append(rowy)
            else:
                X_right.append(rowx)
...
def information_gain(previous_y, current_y):
    ...
    info_gain = 0.45915
    """
    total_entropy = entropy(previous_y)
    H = 0.0
    for i in current_y:
        H = H + 1.0*entropy(i)*len(i)/len(previous_y)
    info_gain = total_entropy - H
    return info_gain