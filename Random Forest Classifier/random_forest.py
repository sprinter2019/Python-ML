...
#            element = np.random.randint(0,n-1)
            indices.append(np.random.randint(0,n-1))
        
        for j in indices:
            samples.append(XX[j][:-1])
            labels.append(XX[j][-1])
    ...
    def fitting(self):
        ...
        for index, myTree in enumerate(self.decision_trees):
            myTree.learn(self.bootstraps_datasets[index], self.bootstraps_labels[index])
            pass      
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for Trees in self.decision_trees:
                    votes.append(Trees.classify(record))
