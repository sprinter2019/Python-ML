...
#    print("End of Neural Network Analysis"); 
    ...
    for i in range(len(learningRate)):
        ...
        randOpt=mlrose.NeuralNetwork(hidden_nodes=[250], activation='tanh', algorithm='genetic_alg', max_iters=maxIter, bias=True,
                                 is_classifier= True, learning_rate=learningRate[i], early_stopping=True, clip_max= 5,
    ...
    for i in range(len(learningRate)):
        ...
        randOpt=mlrose.NeuralNetwork(hidden_nodes=[250], activation='tanh', algorithm='random_hill_climb', max_iters=maxIter, bias=True,
                                 is_classifier= True, learning_rate=learningRate[i], early_stopping=True, clip_max= 5,
    ...
    for i in range(len(learningRate)):
        ...
        randOpt=mlrose.NeuralNetwork(hidden_nodes=[250], activation='tanh', algorithm='simulated_annealing', max_iters=maxIter, bias=True,
                                 is_classifier= True, learning_rate=learningRate[i], early_stopping=True, clip_max= 5,
                                 max_attempts=10, random_state=100)
    ...
    #plt.xlim(0, 100)
    plt.ylim(45, 60)
    plt.plot(learningRate, trainAccuracyGA, 'g*:', label='GA_Train')
    plt.plot(learningRate, testAccuracyGA, 'gs-' , label='GA_Test')
    plt.plot(learningRate, trainAccuracySA, 'b*:', label='SA_Train')
    plt.plot(learningRate, testAccuracySA, 'bs-', label='SA_Test')
    plt.plot(learningRate, trainAccuracyRHC, 'r*:' , label='RHC_Train')
    plt.plot(learningRate, testAccuracyRHC, 'rs-', label='RHC_Test')
    plt.legend(loc='best', fontsize=12)
    plt.savefig('./Plots/NN_opt_accuracyLR.png')
    plt.show(block=False)
        
    
    plt.figure('NN_02')
    plt.title('Training Time vs Learning Rate', fontsize=16);
    plt.ylabel('Training Time (s)', fontsize=14);
    plt.xlabel('Learning Rate', fontsize=14);
    #plt.xlim(0, 100)
    #plt.ylim(40, 65)
    plt.yscale('log')
    plt.plot(learningRate, fitTimeGA, 'go', label='Genetic Algorithm')
    plt.plot(learningRate, fitTimeRHC, 'rs' , label='Random Hill Climbing')
    plt.plot(learningRate, fitTimeSA, 'b*', label='Simulated Annealing')
    plt.legend(loc='best', fontsize=12)
    plt.savefig('./Plots/NN_opt_fitTimeLR.png')
    plt.show()
