...
def decisionTreeAnalysis (x_data, y_data):
    ...
    for i in range(5, 95, 5):
        ...
        dtc=tree.DecisionTreeClassifier(criterion="gini", max_depth=30, random_state=100)
    ...
    for i in range(50, 350, 50):
        adb_Estimator=AdaBoostClassifier(algorithm='SAMME.R',
                   base_estimator=tree.DecisionTreeClassifier(class_weight=None,
                                                         ...
                                                         random_state=100,
                                                         splitter='best'),
                   learning_rate=0.75, n_estimators=i, random_state=100)
    ...
    for i in range(5, 95, 5):
        ...
        adb_tuned=AdaBoostClassifier(algorithm='SAMME.R',
                   base_estimator=tree.DecisionTreeClassifier(class_weight=None,
                                                         ...
                                                         random_state=100,
                                                         splitter='best'),
                   learning_rate=0.75, n_estimators=100, random_state=100)
...
def kNearestNeighbourAnalysis (x_data, y_data):
    ...
    for i in range(1, 20):
        knn_k=KNeighborsClassifier(algorithm='auto', leaf_size=30, metric='minkowski',
                     metric_params=None, n_jobs=None, n_neighbors=i, p=2, weights='uniform')
...
def svmAnalysis (x_data, y_data):
    ...
    for i in range(5, 95, 5):
        x_train, x_test, y_train, y_test = train_test_split(x_data, y_data, test_size=(100-i)/100, random_state=100)
        svm_tuned=SVC(C=10, cache_size=200, class_weight=None, coef0=0.0,
                         decision_function_shape='ovr', degree=1, gamma='scale', kernel='rbf',
                         max_iter=-1, probability=False, random_state=100, shrinking=True, tol=0.001,
                         verbose=False)
...
def nnMlpAnalysis (x_data, y_data):
    ...
    for i in range(50, 500, 50):
        mlp_nSize=MLPClassifier(activation='tanh', alpha=1e-05, batch_size='auto', beta_1=0.9,
    ...
    for i in range(5, 95, 5):
        x_train, x_test, y_train, y_test = train_test_split(x_data, y_data, test_size=(100-i)/100, random_state=100)
        mlp_tuned=MLPClassifier(activation='tanh', alpha=1e-05, batch_size='auto', beta_1=0.9,
                  beta_2=0.999, early_stopping=True, epsilon=1e-08,
                  hidden_layer_sizes=(100,), learning_rate='adaptive',
                  learning_rate_init=0.001, max_iter=200, momentum=0.9,
                  n_iter_no_change=25, nesterovs_momentum=True, power_t=0.5,
